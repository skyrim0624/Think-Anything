...
- 在 Dock 里继续追问、恢复历史对话、查旧笔记、保存卡片、确认全文入库。
...
- `Esc`：折叠 Dock，不清空当前对话。
- 拖动顶部细条：移动 Dock。
- 模型下拉：默认 `gpt-5.5`，可切换到其他已配置模型。
- 思考强度：`极速` 使用 fast/low；`xhigh` 使用 deep/xhigh。
- `历史`：恢复最近的 Dock 对话，继续沿着原来的 session、threadPath 和上下文追问。
- `新对话`：清空当前页面会话，保留新上下文。
...
- `fast`：默认模式，只使用选区、附近上下文、视觉附件和最近对话，不默认查库。
