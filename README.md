...
- VidMark 视频精读：在 YouTube 视频页按 `Option+Shift+V` 打开视频阅读器，自动读取字幕、请求中文精译、生成高能片段，并保存到 Obsidian。
...
- `Option+Shift+V`：在 YouTube 视频页打开 VidMark 视频精读器。
...
- 回答生成中会显示“正在思考”占位消息和动态点，停止/失败会明确改写状态。
- `重试`：重新发送上一问。
- `保存`：按 AI 建议写入卡片。
- `查库`：强制联系旧笔记。
...
VidMark 是 Think Anytime 的视频精读子系统，用来把必看 YouTube 视频变成可跳转、可翻译、可笔记的中文学习材料。

当前 MVP 能力：

- 在 YouTube watch 页面通过 `Option+Shift+V` 打开 VidMark。
- 自动识别视频标题、视频 ID、规范链接和当前播放时间。
- 从 YouTube 页面脚本中读取 caption track，并把 timedtext 字幕转换成带时间戳的字幕句。
- 本地 Bridge 可调用 Codex 生成自然中文精译，并继续生成高能片段。
- 阅读器包含 `字幕`、`高能`、`笔记` 三个视图；点击字幕或片段会跳转视频。
- 点击 `保存` 会把当前视频、字幕、片段和笔记写入 `50-VIDMARK/videos/`，并更新 `50-VIDMARK/index.md`。

运行要求：

- Chrome 扩展需要重新构建并刷新。
- Bridge 需要保持运行，且扩展设置里的 Bridge URL 和 token 要可用。
- Codex CLI 需要可用；如果 Bridge 或 Codex 不可用，VidMark 仍会保留原始字幕阅读能力，但不会生成中文精译和高能片段。

当前边界：

- MVP 优先支持 YouTube；X 视频识别和转写还没有进入可用闭环。
- 只读取页面可发现的 caption track；无字幕视频的 Whisper 转写还未接入。
- 翻译和高能片段依赖本地 Codex，可用性受本机登录和网络状态影响。
...
- `POST /api/vidmark/translate`：把 VidMark 字幕句翻译成简体中文。
- `POST /api/vidmark/highlights`：从 VidMark 字幕中生成高能片段。
- `POST /api/vidmark/save-card`：保存 VidMark 视频卡到 Obsidian。
