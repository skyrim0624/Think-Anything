...
- 在任意网页选中文字后按 `Option+S`，把当前上下文附加到右侧常驻 Dock。
- 在 Dock 里继续追问、查旧笔记、保存卡片、确认全文入库。
- Dock 可折叠、拖动、记住位置；默认收起在页面右下角，不再卡在正文中央。
- 默认提问走极速模式，只发送选区、附近上下文和最近对话；查库和全文判断按需触发。
...
- `Option+S`：展开 Think Anytime Dock，并附加当前选区、图片或视频画面。
